...
public class ModelerController implements RestServiceController<Model, String> {
     ...
     * 我的审核
     */
    @PostMapping("{name}/myAudit")
    public Object myAudit(@PathVariable("name") String userName) {
        List<Task> taskList = taskService.createTaskQuery().taskCandidateUser(userName)
                .orderByTaskCreateTime().desc().list();
        List<Map> vacTaskList = new ArrayList<>();
        for (Task task : taskList) {
            Map vacTask = new HashMap();
            vacTask.put("id", task.getId());
            vacTask.put("name", task.getName());
            vacTask.put("time", task.getCreateTime());
            String instanceId = task.getProcessInstanceId();
            ProcessInstance instance = runtimeService.createProcessInstanceQuery().processInstanceId(instanceId).singleResult();
            vacTask.put("int", instance);
